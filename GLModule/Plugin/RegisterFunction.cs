...
namespace GLModule.PluginJS
{
    public static class RegisterFunction
    {
        ...
        {
            if (!Directory.Exists(Constants.PluginConstants.PathPluginJS)
             || !Constants.PluginConstants.AllowPluginJS)
            { return; }

            Engines.EnginePlugins.AddHostObject("RegisterFunc", new Action<Object, int>(RegisterFunc));

            bool ErroPlugins = false;
            string[] FilesJS = Directory.GetFiles(Constants.PluginConstants.PathPluginJS, "*.js", SearchOption.AllDirectories);
            ...
            if (!ErroPlugins)
                ...
                ConsoleConstants.WriteInConsole("Falha ao carregar alguns plugins", Color.DarkRed);
        ...
        {
            if (SafePlugins.Length == 0 || !Constants.PluginConstants.AllowSafePlugin)
                return;
