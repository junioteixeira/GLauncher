...
namespace GLModule.PluginJS
{
    public static class InvokeFunctions
    {
        ...
        {
            if (!RegisterFunction.FunctionsJS.ContainsKey(functionEnum) || !PluginConstants.AllowPluginJS)
                return;
        ...
        {
            if (!RegisterFunction.SafeFunctionsJS.ContainsKey(functionEnum) || !PluginConstants.AllowSafePlugin)
                return;
        ...
        {
            if (!PluginConstants.AllowAnonymousSafePlugin) { return; }
            try
            {
